...
function save_options() {
  ...
  var displayPrecipitations = document.getElementById("checkBoxPrecipitations")
    .checked;
...
function restore_options() {
  chrome.storage.sync.get(
    ...
    function (items) {
      document.getElementById("calendarPrivateURL").value =
        items.calendarPrivateURL;
      document.getElementById("sogoMailBoxURL").value = items.sogoMailBoxURL;
      document.getElementById("checkBoxUseSOGO").checked =
        items.checkBoxUseSOGO;
      document.getElementById("checkBoxPrecipitations").checked =
        items.displayPrecipitations;
      document.getElementById("checkBoxFahrenheit").checked =
        items.displayFahrenheit;
      for (var i = 0; i < items.rssSources.length; i++) {
        ...
        childDiv.getElementsByClassName("rssSource")[0].value =
          items.rssSources[i];
        childDiv.getElementsByClassName("rssTitle")[0].value =
          items.rssTitles[i];
